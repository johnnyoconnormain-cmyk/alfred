...
export async function addCrewAction(formData: FormData): Promise<void> {
  ...
  if (!name) return;
  await mutate(() => createCrew(business.id, name, String(formData.get('color') ?? 'slot1')));
  revalidatePath('/settings');
  revalidatePath('/schedule');
}
...
export async function addTeamMemberAction(formData: FormData): Promise<void> {
  ...
  const role = String(formData.get('role') ?? 'crew');
  // Hashing is deliberately outside the write: scrypt is slow by design and the
  // write may be replayed.
  ...
  await mutate(() => {
    ...
    run(
      `INSERT INTO users (id, business_id, email, name, phone, role, password_hash, active, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)`,
      [userId, business.id, normalizeEmail(email), name, phone, role, passwordHash, isoNow()],
    );
    if (crewId) {
...
export async function updateFollowUpAction(formData: FormData): Promise<void> {
  ...
  await mutate(() => updateAutomationConfig(business.id, 'quote_follow_up', { stages }));
  revalidatePath('/automations');
}
...
export async function updateAutomationOptionsAction(formData: FormData): Promise<void> {
  ...
  if (config) await mutate(() => updateAutomationConfig(business.id, key, config));
  revalidatePath('/automations');
}
