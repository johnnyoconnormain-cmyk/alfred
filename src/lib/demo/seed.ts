...
import { id, token as makeToken, slugify, withDeterministicIds } from '../ids';
...
/**
 * Demo imagery is generated by a route on request rather than written to disk.
 * Serverless filesystems are read-only outside /tmp, and rendering an SVG is
 * cheaper than storing one.
 */
function writePhoto(_businessId: string, seed: number, phase: 'before' | 'after'): string {
  return demoPhotoUrl(seed, phase);
}
 ...
 */
export function seedDemo(opts: { force?: boolean; deterministic?: boolean } = {}): SeedResult {
  if (opts.deterministic) {
    // Same ids on every rebuild — see `withDeterministicIds`.
    return withDeterministicIds(20260922, () => buildDemo(opts));
  }
  return buildDemo(opts);
}

function buildDemo(opts: { force?: boolean } = {}): SeedResult {
  const db = getDb();
